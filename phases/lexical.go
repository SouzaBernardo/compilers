...
func Lexical(input *string) {
	...
	var font []string = strings.Split(*input, "\n")
	var fontLine string = font[0]
	utils.MoveLookAhead(&pointer, &fontLine, currentLine, len(font), &lookAhead)
	for lookAhead != FIM {
		err := utils.SearchNextToken(&pointer, &fontLine, currentLine, len(font), &lookAhead)
		if err != nil {
			panic(err)
		}
	}
